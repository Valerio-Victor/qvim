...
mapa_insumos <- municipios %>% 
  ...
  tidyr::pivot_longer(cols = !c(name_region,
                      ...
                      names_to = 'insumo',
                      values_to = 'valor') %>% 
  dplyr::arrange(name_region, insumo, desc(valor)) %>% 
...
g1_amostra <- as.data.frame(mapa_total) %>% 
  ...
  ggplot() + 
  geom_col(aes(x = municipios, y = regiao, group = 1, 
...
g2_amostra <- 
  ...
  geom_sf(data = mapa_total, 
          ...
          color = ifelse(mapa_total$name_muni == 'Itajubá', verde_c, roxo_e)) +
  theme_minimal() + 
...
## Análise dos Insumos
```{r echo=FALSE, message=FALSE, warning=FALSE}
mapa_insumos_key <- SharedData$new(mapa_insumos, key = ~name_muni)

g1_insumos <- mapa_insumos_key %>% 
  ggplot() + 
  geom_jitter(aes(x = valor, y = insumo, group = 1,
                  text = paste0('Município: ', name_muni,
                                '<br>Região: ', name_region,
                                '<br>Insumo: ', insumo,
              ...
              color = ifelse(mapa_insumos$name_muni == 'Itajubá', verde_c, roxo_e)) +
  scale_x_continuous(labels = scales::number_format(big.mark = '.')) + 
  labs(x = 'R$/Habitante',
       y = '') + 
  theme_classic()

g1_insumos <- plotly::ggplotly(g1_insumos, tooltip = 'text') %>%
  plotly::config(modeBarButtonsToRemove = plotly_layout)
